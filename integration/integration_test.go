...
func (s *IntSuite) TestDisconnectScenarios(c *check.C) {
	...
	testCases := []disconnectTestCase{
		...
		{ // verify that concurrent session limits are applied when recording at node
			recordingMode: services.RecordAtNode,
			options: services.RoleOptions{
		...
		{ // verify that concurrent session limits are applied when recording at proxy
			recordingMode: services.RecordAtProxy,
			options: services.RoleOptions{
				ForwardAgent:          services.NewBool(true),
				MaxConcurrentSessions: 1,
		...
		{ // verify that lost connections to auth server terminate controlled sessions
			recordingMode: services.RecordAtNode,
			options: services.RoleOptions{
				MaxConcurrentSessions: 1,
			},
			...
			// use postFunc to wait for the semaphore to be acquired, and then shut down the
			// auth server.
...
func (s *IntSuite) runDisconnectTest(c *check.C, tc disconnectTestCase) {
	...
	if tc.concurrentSessions < 1 {
		// test cases that don't specify concurrentSessions are single-session tests.
		tc.concurrentSessions = 1
	}

	for i := 0; i < tc.concurrentSessions; i++ {
		person := NewTerminal(250)
